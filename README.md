...
In addition, errors from the underlying node-ftp library may be rejected unchanged through method calls.
...
* **writeData**(handle <_Buffer_>, buffer <_Buffer_>, offset <_integer_>, length <_integer_>, position <_integer_>):
...
* **rmdir**(path <_string_>): Removes the directory at `path`. Returned promise resolves to _undefined_.
